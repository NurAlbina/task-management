...
const TaskFormPage = () => {
  ...
  const [selectedFiles, setSelectedFiles] = useState([]); // Artık sadece yeni eklenenler için kullanılacak
  const [existingAttachments, setExistingAttachments] = useState([]); // Sunucudan gelen + yeni eklenen dosyalar
  ...
  const validateDate = (date) => {
    ...
    if (selectedDate.getFullYear() > maxYear) {
      return {
        valid: false,
        message: `❌ Tarih ${maxYear} yılından ileri olamaz`,
      };
    }
  ...
  const handleFileChange = (e) => {
    ...
    // Yeni dosyaları existingAttachments'a ekle (tip kontrolü ile)
    setExistingAttachments((prev) => [
      ...
      ...files.map((file) => ({
        ...
        fileName: file.name,
        fileSize: file.size,
        fileUrl: URL.createObjectURL(file), // geçici önizleme için
        isNew: true,
      })),
    ]);
  ...
  // Dosya silme fonksiyonu (hem yeni hem eski için)
  const handleRemoveExistingFile = (fileUrl) => {
    setExistingAttachments((prev) =>
      prev.filter((att) => att.fileUrl !== fileUrl)
    );
    // Eğer eski dosya ise backend'e bildir
    const found = existingAttachments.find(
      (att) => att.fileUrl === fileUrl && !att.isNew
    );
    ...
    try {
      ...
      setError(
        error.response?.data?.message ||
          (isEditMode ? "Görev güncellenemedi" : "Görev eklenemedi")
      );
    } finally {
      setLoading(false);
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                >
                  ...
                  <option className="bg-slate-800" value="Shopping">
                    🛒 Shopping
                  </option>
                  <option className="bg-slate-800" value="Health">
                    ❤️ Health
                  </option>
                  <option className="bg-slate-800" value="Other">
                    📌 Other
                  </option>
                </select>
              </div>
              ...
              {isEditMode && (
                <div>
                  ...
                  >
                    ...
                    <option className="bg-slate-800" value="in-progress">
                      🔄 In Progress
                    </option>
                    <option className="bg-slate-800" value="completed">
                      ✅ Completed
                    </option>
                  </select>
                </div>
            ...
            {existingAttachments.length > 0 && (
              <div className="space-y-2 border border-white/10 p-4 rounded-xl bg-black/10">
                ...
                {existingAttachments.map((file, index) => (
                  <div
                    key={index}
                    className="flex justify-between items-center bg-white/5 border border-white/10 p-2 rounded-lg group hover:border-teal-500/30 transition-all"
                  >
